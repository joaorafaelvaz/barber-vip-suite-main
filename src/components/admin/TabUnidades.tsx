...
export function TabUnidades() {
  const { t } = useI18n();
  const { isMaster, isOrgAdmin } = usePermissions();
  const { organizations } = useOrgs();
  const canManage = isMaster || isOrgAdmin;
  ...
  // Get filtered units based on org filter
  const filteredUnits = selectedOrgFilter 
    ? units.filter(u => u.org_id === selectedOrgFilter)
    : units;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-foreground">{t('nav.unidades')}</h2>
          <p className="text-sm text-muted-foreground">Gerencie as unidades de todas as organizações</p>
        </div>
      ...
      <Card className="bg-card/50 backdrop-blur-sm border-border/50">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-base">{filteredUnits.length} Unidades</CardTitle>
            ...
            <>
              ...
              <div className="md:hidden space-y-3">
                {filteredUnits.map((unit) => (
                  <div key={unit.id} className="p-4 rounded-lg border border-border/50 bg-muted/20 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-foreground">{unit.nome}</span>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
                      {isMaster && (
                        <div className="flex items-center gap-1">
                          ...
                          <span>{unit.org_nome}</span>
                        </div>
                      )}
                      <div className="flex items-center gap-1">
                        <Users className="h-3.5 w-3.5" />
                        <span>{unit.user_count || 0} usr.</span>
                      </div>
                      <span>{format(new Date(unit.created_at), 'dd/MM/yyyy', { locale: ptBR })}</span>
                    </div>
                    {canManage && (
                      <div className="flex items-center gap-2 pt-1">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleToggleStatus(unit)} className="h-8 w-8 p-0">
                          {unit.status === 'ativo' ? <PowerOff className="h-4 w-4 text-destructive" /> : <Power className="h-4 w-4 text-green-500" />}
                        </Button>
                      </div>
                    )}
                  </div>

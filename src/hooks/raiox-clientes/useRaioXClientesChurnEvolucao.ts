...
export interface ChurnEvolucaoItem {
  ano_mes: string;
...
export interface ChurnEvolucaoData {
  meta: {
    inicio: string;
    fim: string;
    base_mode: string;
  ...
  series: ChurnEvolucaoItem[];
}

export function useRaioXClientesChurnEvolucao(filters: RaioXComputedFilters, configOverrides?: ChurnConfigOverrides, enabled = true) {
  const [data, setData] = useState<ChurnEvolucaoData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data: result, error: rpcError } = await supabase.rpc(
        'rpc_raiox_clientes_churn_evolucao_v2' as any,
        {
      ...
      if (rpcError) {
        ...
        setData(result as unknown as ChurnEvolucaoData);
      }
    } catch (err: any) {

...
import type { ChurnConfigOverrides } from './useRaioXClientesChurn';

export interface ChurnEvolucaoBarbeiroItem {
  ano_mes: string;
...
export interface ChurnEvolucaoBarbeiroData {
  meta: {
    inicio: string;
    fim: string;
    base_mode: string;
  ...
  series: ChurnEvolucaoBarbeiroItem[];
}

export function useRaioXClientesChurnEvolucaoBarbeiro(filters: RaioXComputedFilters, configOverrides?: ChurnConfigOverrides, enabled = true) {
  const [data, setData] = useState<ChurnEvolucaoBarbeiroData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data: result, error: rpcError } = await supabase.rpc(
        'rpc_raiox_clientes_churn_evolucao_barbeiro_v1' as any,
        {
      ...
      if (rpcError) {
        ...
        setData(result as unknown as ChurnEvolucaoBarbeiroData);
      }
    } catch (err: any) {
  ...
  useEffect(() => { if (!enabled) return; fetchData(); }, [fetchData, enabled]);

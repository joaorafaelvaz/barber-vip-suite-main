...
BEGIN
  ...
  v_sply_dias := (v_sply_fim::date - v_sply_inicio::date) + 1;

  -- MoM
  v_mom_fim := v_inicio - 1;
  v_mom_inicio := v_mom_fim - (v_dias - 1);
  v_mom_dias := v_dias;
  ...
  -- Avg 6m
  SELECT
    COALESCE(SUM(valor_faturamento), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg6_fim - v_avg6_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1),
    COALESCE(SUM(CASE WHEN is_base THEN valor_faturamento ELSE 0 END), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg6_fim - v_avg6_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1),
    COALESCE(SUM(CASE WHEN is_extra THEN valor_faturamento ELSE 0 END), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg6_fim - v_avg6_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1),
    COALESCE(SUM(CASE WHEN is_produto THEN valor_faturamento ELSE 0 END), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg6_fim - v_avg6_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1)
  ...
  -- Avg 12m
  SELECT
    COALESCE(SUM(valor_faturamento), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg12_fim - v_avg12_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1),
    COALESCE(SUM(CASE WHEN is_base THEN valor_faturamento ELSE 0 END), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg12_fim - v_avg12_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1),
    COALESCE(SUM(CASE WHEN is_extra THEN valor_faturamento ELSE 0 END), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg12_fim - v_avg12_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1),
    COALESCE(SUM(CASE WHEN is_produto THEN valor_faturamento ELSE 0 END), 0) / GREATEST(EXTRACT(EPOCH FROM (v_avg12_fim - v_avg12_inicio + 1) * interval '1 day') / EXTRACT(EPOCH FROM interval '30 days'), 1)

...
CREATE OR REPLACE FUNCTION public.rpc_raiox_clientes_churn_evolucao_v1(
  ...
  p_churn_dias_sem_voltar integer DEFAULT 90,
  p_risco_min_dias integer DEFAULT 45,
  p_risco_max_dias integer DEFAULT 90,
  p_cadencia_min_visitas integer DEFAULT 3,
  p_resgate_dias_minimos integer DEFAULT 60,
  p_atribuicao_modo text DEFAULT 'ultimo'::text,
  p_atribuicao_janela_meses integer DEFAULT 6,
  p_base_mode text DEFAULT 'TOTAL_COM_CORTE'::text,
  p_base_corte_meses integer DEFAULT 12,
  p_ref_mode text DEFAULT 'FIM_PERIODO'::text
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  WHILE v_month_start <= v_fim LOOP
    ...
    -- FIX Bug 2: Skip months with no sales data
    ...
    -- FIX Bug 1 & 3: base_ativa now INCLUDES em_risco (all clients within churn_dias_sem_voltar)
    -- em_risco is a SUBSET of base_ativa for visualization only
    SELECT

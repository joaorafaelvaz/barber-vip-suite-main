-- Drop existing function first and recreate it properly
DROP FUNCTION IF EXISTS rpc_servicos_analise(DATE, DATE, TEXT, TEXT, TEXT);

-- Create RPC function for services analysis
...
BEGIN
  -- Validate agrupamento parameter
  ...
  SELECT json_build_object(
    'faturamento_total', COALESCE(SUM(valor_bruto), 0),
    'quantidade_total', COUNT(*),
    'ticket_medio', CASE WHEN COUNT(*) > 0 THEN COALESCE(SUM(valor_bruto), 0) / COUNT(*) ELSE 0 END,
    'top_servicos', (
      SELECT json_agg(
        json_build_object(
          'nome', produto,
      ...
      FROM (
        SELECT 
          produto,
          SUM(valor_bruto) as total_faturamento
        FROM vendas_api_raw v
        LEFT JOIN dimensao_produtos dp ON v.produto = dp.produto
        WHERE venda_data_ts::date BETWEEN p_data_inicio AND p_data_fim

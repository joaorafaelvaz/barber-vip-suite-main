-- =============================================================
-- MIGRAÇÃO: Atualizar RPC para filtrar colaboradores com faturamento no período
-- ADICIONA: Busca última data com faturamento no mês atual
-- =============================================================

-- 1. CRIAR FUNÇÃO para buscar última data com faturamento do mês atual
...
BEGIN
  ...
  WHERE venda_dia::date >= v_month_start
    AND venda_dia::date <= CURRENT_DATE
    AND valor_faturamento > 0;
  
  -- Se não encontrou, retorna primeiro dia do mês
...
-- 2. ATUALIZAR RPC para filtrar colaboradores COM faturamento no período
...
BEGIN
  ...
  -- (Filtra apenas quem tem faturamento no período selecionado)
  -- ===========================================
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(

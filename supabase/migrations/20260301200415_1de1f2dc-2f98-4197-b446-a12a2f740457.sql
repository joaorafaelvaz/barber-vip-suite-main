...
-- ============================================================
-- Fix rpc_raiox_clientes_churn_evolucao_v1
-- 1) base_ativa now INCLUDES em_risco (last_seen <= churn_dias_sem_voltar)
-- 2) churn denominators now use total universe (base_ativa + perdidos)
--    which already includes em_risco since base_ativa was redefined
-- ============================================================
...
BEGIN
  ...
  WHILE v_month_start <= v_fim LOOP
    ...
    -- FIX: base_ativa now INCLUDES em_risco (all clients with last_seen <= churn_dias_sem_voltar)
    -- em_risco is a SUBSET of base_ativa for visualization purposes
    SELECT

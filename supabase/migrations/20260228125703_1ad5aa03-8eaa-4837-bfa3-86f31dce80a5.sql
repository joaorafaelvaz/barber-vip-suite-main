...
CREATE OR REPLACE FUNCTION public.rpc_clientes_novos_resumo(
  ...
  p_janela_conversao integer DEFAULT 60,
...
BEGIN
  WITH
  -- Use dimensao_clientes.first_seen instead of computing MIN(venda_dia) over millions of rows
  novos AS (
  ...
  -- Only scan vendas for the novos clients, from their first_seen to first_seen + janela
  vendas_novos AS (
    SELECT
      v.cliente_id,
      v.colaborador_id,

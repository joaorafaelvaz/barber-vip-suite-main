...
-- Add base_distribuicao filtered by last_seen >= v_ref - 365
-- Distributions use base_distribuicao; KPIs keep full base (cx)
-- =====================================================
...
BEGIN
  ...
  -- FULL client base from dimensao_clientes (for KPIs of risk/lost)
  base_completa AS (
    SELECT
      dc.cliente_id,
      dc.first_seen,
  ...
  -- FILTERED base for distributions: only clients active in last 365 days
  base_distribuicao AS (
    SELECT * FROM cx
    WHERE last_seen >= (v_ref - 365)
  ),
  ...
  -- DISTRIBUTIONS now use base_distribuicao (filtered by 365 days)
  dist_perfil AS (
...
-- =====================================================
-- FIX: rpc_raiox_overview_drill_v1
-- Add last_seen >= v_ref - 365 filter to match distributions
-- =====================================================

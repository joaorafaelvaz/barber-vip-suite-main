...
CREATE OR REPLACE FUNCTION public.rpc_raiox_clientes_cadencia_drill_v1(
  p_inicio text, p_fim text, p_janela_dias integer DEFAULT 60,
  p_colaborador_id text DEFAULT NULL, p_excluir_sem_cadastro boolean DEFAULT true,
  p_tipo text DEFAULT 'ASSIDUO', p_valor text DEFAULT '',
  p_limit integer DEFAULT 500, p_base_mode text DEFAULT 'TOTAL_COM_CORTE',
  p_base_corte_meses integer DEFAULT 24, p_ref_mode text DEFAULT 'FIM_FILTRO',
  p_cadencia_meses_analise integer DEFAULT 12, p_cadencia_min_visitas integer DEFAULT 3,
  p_ratio_muito_frequente_max numeric DEFAULT 0.8, p_ratio_regular_max numeric DEFAULT 1.2,
  p_ratio_espacando_max numeric DEFAULT 1.8, p_ratio_risco_max numeric DEFAULT 2.5,
  p_one_shot_aguardando_max_dias integer DEFAULT 45, p_one_shot_risco_max_dias integer DEFAULT 90,
  p_atribuicao_modo text DEFAULT 'ULTIMO', p_atribuicao_janela_meses integer DEFAULT 12
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  janela_valor AS (
    -- Revenue in analysis window (v_cadencia_desde..v_ref) — same window as classification
    SELECT v.cliente_id,
           SUM(COALESCE(v.valor_bruto, 0)) AS valor_janela
    FROM vendas_api_raw v
    JOIN base_clientes bc ON bc.cliente_id = v.cliente_id
    WHERE v.venda_data_ts IS NOT NULL
      AND v.venda_data_ts::date BETWEEN v_cadencia_desde AND v_ref
  ...
  cliente_resumo AS (
    SELECT bc.cliente_id, bc.cliente_nome, bc.telefone, a.colaborador_nome, a.colaborador_id,
           ...
           (SELECT COUNT(DISTINCT vv.dia) FROM visitas vv WHERE vv.cliente_id = bc.cliente_id) AS total_visitas_janela,
           COALESCE(jv.valor_janela, 0) AS valor_janela,
    ...
    LEFT JOIN janela_valor jv ON jv.cliente_id = bc.cliente_id
  ),
  classificado AS (
    SELECT cr.*,
           v_ref - cr.ultima_visita AS dias_sem_vir,
           CASE WHEN cr.cadencia_dias IS NOT NULL AND cr.cadencia_dias > 0
                  THEN ROUND(((v_ref - cr.ultima_visita)::numeric / cr.cadencia_dias::numeric), 2) ELSE NULL END AS ratio,
           CASE
             WHEN cr.ultima_visita IS NULL THEN 'PERDIDO'
             WHEN cr.total_visitas_janela = 1 THEN
             ...
             WHEN cr.cadencia_dias IS NULL OR cr.cadencia_dias <= 0 THEN
               CASE WHEN (v_ref - cr.ultima_visita) <= p_one_shot_aguardando_max_dias THEN 'REGULAR'
                    WHEN (v_ref - cr.ultima_visita) <= p_one_shot_risco_max_dias THEN 'EM_RISCO'
                    ELSE 'PERDIDO' END
             WHEN ((v_ref - cr.ultima_visita)::numeric / cr.cadencia_dias::numeric) <= p_ratio_muito_frequente_max THEN 'ASSIDUO'
             WHEN ((v_ref - cr.ultima_visita)::numeric / cr.cadencia_dias::numeric) <= p_ratio_regular_max THEN 'REGULAR'
             WHEN ((v_ref - cr.ultima_visita)::numeric / cr.cadencia_dias::numeric) <= p_ratio_espacando_max THEN 'ESPACANDO'
             WHEN ((v_ref - cr.ultima_visita)::numeric / cr.cadencia_dias::numeric) <= p_ratio_risco_max THEN 'EM_RISCO'
             ELSE 'PERDIDO'
  ...
  SELECT jsonb_build_object(
    ...
    'rows', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'ratio', l.ratio,
        'visitas_total', l.total_visitas_janela,
        'valor_total', l.valor_janela
      ))
      FROM limited l
    ), '[]'::jsonb)
  ) INTO v_result;

...
-- ============================================================
-- RPC: rpc_clientes_painel_completo
-- Returns comprehensive JSONB with all client panel data
-- Uses TEXT params for PostgREST compatibility
-- ============================================================
...
BEGIN
  ...
  WITH
  -- 1) Base: all sales in the period, optionally filtered by collaborator
  base AS (
    SELECT
      v.cliente_id,
      v.cliente_nome,
  ...
  -- 2) Per-client aggregation in the period
  clientes_periodo AS (
    SELECT
      b.cliente_id,
      ...
      SUM(b.valor_bruto) AS valor_total,
      COUNT(DISTINCT b.colaborador_id) AS qtd_barbeiros
    FROM base b
    GROUP BY b.cliente_id
  ),

  -- 3) Identify truly new clients (first visit EVER is within the period)
  ...
  -- 4) Status calculation (same cadence logic as rpc_clientes_status_dinamico)
  ...
  -- 8) Per barber stats (only when not filtered by barber)
  por_barbeiro AS (
    SELECT jsonb_agg(jsonb_build_object(

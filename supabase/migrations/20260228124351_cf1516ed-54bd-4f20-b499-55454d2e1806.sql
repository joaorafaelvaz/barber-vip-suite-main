...
-- Fix: use valor_faturamento (COALESCE(valor_liquido, valor_bruto, 0)) instead of valor_bruto
-- Fix: ticket_medio_recorrente should be per-visit (per client), not per-item-row
...
BEGIN
  ...
  metricas_novo AS (
    SELECT
      ...
      -- Ticket primeira visita: soma de valor_faturamento no dia do first_seen
  ...
  -- Ticket médio recorrente: gasto total / dias de visita (por visita, não por item)
...
-- ============================================================
-- RPC 2: rpc_clientes_novos_lista (fix valor_faturamento)
-- ============================================================
...
BEGIN
  ...
  base AS (
    SELECT
      ...
      v.tipo_colaborador, v.venda_dia, v.valor_faturamento, v.venda_id
    FROM vw_vendas_kpi_base v
  ...
  enriched AS (
    SELECT
      ...
      -- Gasto: soma de valor_faturamento na janela de conversão
      (SELECT COALESCE(SUM(valor_faturamento), 0) FROM base bb WHERE bb.cliente_id = n.cliente_id

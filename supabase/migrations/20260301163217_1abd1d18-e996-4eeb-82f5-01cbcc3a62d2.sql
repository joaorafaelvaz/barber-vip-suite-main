...
DROP FUNCTION IF EXISTS public.rpc_raiox_clientes_overview_v1(text, text, text, text, int, text, boolean, text, text, int, int);
...
BEGIN
  ...
  -- Usa dimensao_clientes diretamente com filtro temporal de p_status12m_meses a partir de v_ref
  ...
  dist_cadencia AS (
    ...
    FROM (
      SELECT status, COUNT(*) AS qtd FROM (
        SELECT CASE
          WHEN dias_sem_vir <= p_janela_dias THEN 'EM_DIA'
          WHEN dias_sem_vir <= v_risco_min THEN 'ALERTA'
    ...
    FROM (
      SELECT macro, COUNT(*) AS qtd FROM (
        SELECT CASE
          ...
          WHEN dias_sem_vir <= v_risco_max AND visitas_total > 1 THEN 'ATIVO'
          WHEN dias_sem_vir > v_risco_max AND visitas_total > 1 THEN 'INATIVO'
          ELSE 'INDEFINIDO'
        END AS macro
        FROM base_distribuicao

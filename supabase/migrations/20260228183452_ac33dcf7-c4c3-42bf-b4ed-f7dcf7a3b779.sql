...
-- Fix ROUND(double precision, integer) error by adding ::numeric casts
...
BEGIN
    ...
    FROM (
      SELECT
        ...
        jsonb_agg(jsonb_build_object(
          ...
          'cadencia_dias', ROUND(COALESCE(f.cadencia_dias, 0)::numeric, 1)
  ...
  resumo AS (
    SELECT jsonb_build_object(
      'media_dias_sem_vir', ROUND(COALESCE(AVG(f.dias_sem_vir::numeric), 0), 1),
      'media_frequencia', ROUND(COALESCE(AVG(f.dias_distintos::numeric), 0), 1),
      'valor_total', ROUND(COALESCE(SUM(f.valor_total)::numeric, 0), 2),
      'ticket_medio', ROUND(COALESCE(AVG(f.valor_total::numeric / NULLIF(f.atendimentos, 0)), 0), 2)
    ) AS val
    FROM filtered f
  )

...
BEGIN
  ...
  novos AS (
    ...
    JOIN dimensao_clientes dc ON dc.cliente_id = cp.cliente_id
    ...
    FROM (
      SELECT TO_CHAR(b.venda_dia, 'YYYY-MM') AS ano_mes,
        ...
        COUNT(DISTINCT b.cliente_id) FILTER (
          ...
          AND TO_CHAR(dc.first_seen, 'YYYY-MM') = TO_CHAR(b.venda_dia, 'YYYY-MM')
        ) AS clientes_novos,
        COUNT(DISTINCT b.venda_id) AS atendimentos,
        SUM(b.valor_faturamento) AS valor
      FROM base b
      LEFT JOIN dimensao_clientes dc ON dc.cliente_id = b.cliente_id

...
DECLARE
  ...
  v_atual_dias_trab int := 0;
  v_sply numeric := 0;
  v_sply_base numeric := 0;
  v_sply_extras numeric := 0;
  v_sply_produtos numeric := 0;
  v_sply_dias_trab int := 0;
  v_mom numeric := 0;
  v_mom_base numeric := 0;
  v_mom_extras numeric := 0;
  v_mom_produtos numeric := 0;
  v_mom_dias_trab int := 0;
  v_avg6 numeric := 0;
  v_avg6_base numeric := 0;
  v_avg6_extras numeric := 0;
  v_avg6_produtos numeric := 0;
  v_avg6_dias_trab numeric := 0;
  v_avg12 numeric := 0;
  v_avg12_base numeric := 0;
  v_avg12_extras numeric := 0;
  v_avg12_produtos numeric := 0;
  v_avg12_dias_trab numeric := 0;
  v_sply_var numeric;
  v_mom_var numeric;
BEGIN
  v_dias := (v_fim - v_inicio) + 1;

  -- Detect full-month period
  v_is_full_month := (
    EXTRACT(DAY FROM v_inicio) = 1
    AND v_fim = (date_trunc('month', v_fim) + INTERVAL '1 month - 1 day')::date
  );

  IF v_is_full_month THEN
    -- SPLY: same month, previous year (full month)
    v_sply_inicio := (date_trunc('month', v_inicio) - INTERVAL '1 year')::date;
    v_sply_fim := (v_sply_inicio + INTERVAL '1 month - 1 day')::date;
    -- MoM: previous month (full month)
    v_mom_inicio := (date_trunc('month', v_inicio) - INTERVAL '1 month')::date;
    v_mom_fim := (v_inicio - INTERVAL '1 day')::date;
  ELSE
  ...
  SELECT
    ...
    COUNT(DISTINCT venda_dia)
  INTO v_atual, v_atual_base, v_atual_extras, v_atual_produtos, v_atual_dias_trab
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN is_produto THEN valor_faturamento ELSE 0 END), 0),
    COUNT(DISTINCT venda_dia)
  INTO v_sply, v_sply_base, v_sply_extras, v_sply_produtos, v_sply_dias_trab
  FROM vw_vendas_kpi_base
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN is_produto THEN valor_faturamento ELSE 0 END), 0),
    COUNT(DISTINCT venda_dia)
  INTO v_mom, v_mom_base, v_mom_extras, v_mom_produtos, v_mom_dias_trab
  FROM vw_vendas_kpi_base
  ...
  SELECT
    ...
    ROUND(COUNT(DISTINCT venda_dia)::numeric / v_avg6_meses, 1)
  INTO v_avg6, v_avg6_base, v_avg6_extras, v_avg6_produtos, v_avg6_dias_trab
  FROM vw_vendas_kpi_base
  ...
  SELECT
    ...
    ROUND(COUNT(DISTINCT venda_dia)::numeric / v_avg12_meses, 1)
  INTO v_avg12, v_avg12_base, v_avg12_extras, v_avg12_produtos, v_avg12_dias_trab
  FROM vw_vendas_kpi_base
  ...
  RETURN json_build_object(
    ...
    'atual_dias', v_dias,
    'atual_dias_trabalhados', v_atual_dias_trab,
    'sply_total', v_sply,
    ...
    'sply_dias_trabalhados', v_sply_dias_trab,
    ...
    'mom_dias', v_mom_dias,
    'mom_dias_trabalhados', v_mom_dias_trab,
    ...
    'avg_6m_dias_trabalhados', v_avg6_dias_trab,
    ...
    'avg_12m_dias_trabalhados', v_avg12_dias_trab,
